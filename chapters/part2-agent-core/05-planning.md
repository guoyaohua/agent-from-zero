...
![Planning 检查点账本](../assets/part2-planning-checkpoint-ledger.svg)

可以把计划看成检查点账本,而不是任务清单。每个阶段至少要记录目标版本、依赖、假设和验证方式。

目标版本防止漂移。用户目标一旦变更,旧计划不能无声继续执行。依赖关系防止乱序,例如没有读取失败日志就直接修改代码,往往只是猜。假设字段防止模型把“可能原因”写成“已确认事实”。验证字段则告诉 runtime 什么证据出现才算完成,例如测试通过、引用校验通过、用户确认、工具返回成功。

检查点账本还要定义重规划触发器:目标变化、关键假设被反证、依赖阻塞、预算不足、风险升级、连续无进展。触发重规划不代表推翻一切,而是基于已有 state 调整后续路线。好的规划系统不是一次写出完美计划,而是知道什么时候旧计划已经不再可靠。
