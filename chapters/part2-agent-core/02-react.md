...
ReAct 的上限不是 Thought 写得多漂亮,而是 Observation 是否能真正约束下一步。如果工具返回只是自然语言大段文本,没有来源、没有结构、没有信任标签,模型很容易把观察当成又一段待解释文本,而不是可更新状态的事实。

![ReAct 观察质量账本](../assets/part2-react-observation-quality-ledger.svg)

可以从四个维度评估 Observation。

第一,是否可验证。观察最好带 `source_id`、`artifact_ref`、工具调用 ID、时间戳或可回放 fixture。没有这些,最终回答很难证明自己基于真实观察。

第二,是否可行动。好的观察会告诉系统下一步可能是什么:继续读取、改 query、修参数、停止、请求用户。坏观察只是一段摘要,模型读完后仍然不知道状态该怎么变。

第三,是否保留信任级别。工具返回可能来自可信数据库,也可能来自外部网页、邮件或 issue。ReAct 必须区分 evidence 和 instruction,否则外部内容会在观察链路里获得指令权。

第四,是否改变 state。如果一条 observation 不能新增事实、否定假设、暴露错误或触发停止,它就不应该让 Loop 原样继续。工程上可以记录 `observation_delta`:新增了什么、删除了什么假设、降低了什么不确定性。

这也是 ReAct 和普通“边想边查”的分界线。ReAct 的真正价值是让外部世界不断校正模型,而不是让模型把外部世界继续写成一段自我解释。
