...
![ACI 工具粒度账本](../assets/part2-aci-tool-granularity-ledger.svg)

判断工具粒度时,可以问四个问题。

第一,模型是否需要猜业务规则? 如果工具太底层,模型要自己决定先查什么、怎么组合字段、什么时候写入,错误会被放大。好的 ACI 把稳定业务规则封进工具,只把必要选择留给模型。

第二,参数能否从可信来源得到? 一个工具如果要求模型填写大量系统本来知道的字段,就会诱发编造参数。工具应尽量从 state、用户输入或可信工具结果中取参数,而不是让模型补全内部 ID、权限字段和环境配置。

第三,返回是否能驱动下一步? 太粗的工具会隐藏中间状态,太细的工具会产生碎片化 observation。合适的工具返回应该包含摘要、结构化字段、artifact_ref、错误类型和建议下一步。

第四,副作用是否可控? 如果一个工具既能读又能写、既能草稿又能提交,它会让 Harness 很难判断风险。高风险动作最好拆成 preview/draft/commit,让确认和审计有明确位置。

所以 ACI 的目标不是把后端能力原样暴露给模型,而是设计一组 Agent 能可靠理解、可靠调用、可靠恢复的动作表面。
