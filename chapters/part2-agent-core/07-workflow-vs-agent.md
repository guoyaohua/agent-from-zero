...
![Workflow 与 Agent 选型矩阵](../assets/part2-workflow-vs-agent-decision.svg)
...
![Workflow 与 Agent 自主性阶梯](../assets/part2-workflow-agent-autonomy-ladder.svg)

这张阶梯图能帮助你避免“要么全自动,要么不用 Agent”的二分法。很多系统最有价值的位置其实是 L2 或 L3:模型可以查资料、读文件、运行只读工具,也可以创建草稿或补丁,但最终高风险动作仍由 workflow、测试、策略和用户确认控制。

自主性不是模型能力的单独函数,还取决于系统能否控制后果。如果没有 trace,不要上 L2;没有权限和预算,不要上 L3;没有幂等、审批、回滚和监控,不要上 L4。工程成熟度决定了可以安全开放多少自主性。

![Workflow 到 Agent 自主性门禁](../assets/part2-workflow-agent-autonomy-gates.svg)

提高自主性本质上是在给模型更多权力:更多工具、更长时间、更大状态影响、更强副作用。每升一级,都应该同步补门禁。

从 L1 到 L2,要补只读工具的 schema、权限、trace 和错误协议。否则模型虽然能查资料,但你不知道它查到了什么、是否越权、是否误读。

从 L2 到 L3,要补草稿、diff、preview、预算和回滚。低风险写操作也需要让用户或系统看见将要改变什么,不能把“生成草稿”和“提交执行”混在一起。

从 L3 到 L4,要补人工确认、幂等、审计、事故响应和安全 eval。高风险动作一旦执行,错误成本会明显上升,不能只靠模型“更谨慎”。

从 L4 到长期自治,还要补 checkpoint、目标复审、无进展停止、权限续期和 owner。长期运行不是把 while 循环跑久一点,而是把状态、预算、权限和责任都持久化。

所以“Workflow 还是 Agent”不是二选一。更合理的问题是:当前系统已经具备哪些门禁,因此最多能开放到哪一级自主性。权力应该跟随治理能力增长,而不是跟随想象力增长。
...
| 成熟度 | 系统能力 | 建议自主性 |
| --- | --- | --- |
| M0 原型 | 只有 prompt 和手工测试 | L0-L1 |
| M1 可观测 | 有日志、trace、基本评估 | L1-L2 |
| M2 工具化 | 有只读工具、schema、权限 | L2 |
| M3 可恢复 | 有预算、停止、错误恢复、回滚 | L2-L3 |
| M4 生产级 | 有审计、监控、红队、安全策略 | L3,谨慎 L4 |

如果系统还在 M0,不要让 Agent 做 L3/L4 动作。不是模型一定做不到,而是你没有能力发现和控制它做错时的后果。
...
![从 Workflow 渐进演化到混合 Agent](../assets/part2-workflow-agent-hybrid-evolution.svg)

渐进式路线的好处是每一步都能验证收益。先让模型输出结构化结果,你能评估格式和字段;再接入只读工具,你能评估事实 grounding;再用 workflow 固定主路径,你能评估端到端质量;最后只把异常、不确定、多轮探索的节点 Agent 化。这样系统的每一层都有观测和回退方式。

这也解释了为什么许多成功的 Agent 产品看起来并不“全自主”。它们往往是外层 workflow 很强,内部少数节点很智能。用户感受到的是任务被推进,工程上依赖的却是清晰边界、可检查输出和逐步开放的自主性。

## 异常路径优先 Agent 化

一个实用策略是:主路径先 Workflow,异常路径再 Agent。
...
这些才交给 Agent 分析、查资料、生成解释或请求人工。

这样做的好处是:大部分请求走低成本稳定路径,复杂请求才消耗 Agent 的灵活性。
...
到这里,你已经能判断什么时候用 Workflow、什么时候用更自主的 Agent。但生产系统还需要进一步拆清 Prompt、Context、Harness 和 Loop 的边界。下一章会把这四层放在一起比较,帮助你判断失败到底应该在哪一层修。
