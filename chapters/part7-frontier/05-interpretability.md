...
![解释报告访问分层](../assets/part7-interpretability-access-layers.svg)

上图把解释按访问角色分层。用户通常需要结论、引用、已执行动作和剩余风险;工程师需要 span、state diff、工具错误和版本信息;审计需要身份、权限、确认、audit id 和责任边界;研究者则需要匿名样本、失败类型和机制假设。每一层需要的解释深度不同。

这点很关键,因为解释也可能泄露敏感信息。完整 prompt、隐藏策略、长期记忆、第三方秘密、其他用户数据,都不应该为了“解释得更完整”而默认暴露。好的解释系统既能支持复盘,也能按角色裁剪证据,把可验证性和最小披露原则同时保住。
...
| 等级 | 访问者 | 内容 |
| --- | --- | --- |
| L1 用户解释 | 当前用户 | 结论、证据摘要、已执行动作、下一步 |
| L2 调试解释 | 工程团队 | trace span、工具结果、state diff、失败归因 |
| L3 审计解释 | 安全/合规 | 身份、权限、确认、策略决策、责任链 |
| L4 研究解释 | 模型/安全研究 | 脱敏样本、机制分析、聚合统计 |

如果解释系统没有访问控制,团队常常会在两种坏选择之间摇摆:要么解释过浅,无法修复问题;要么解释过深,泄露敏感上下文。分层能让解释既有用又可治理。

![解释访问决策树](../assets/part7-interpretability-access-decision.svg)

访问控制不能只按“内部/外部”粗分。更实用的是把解释请求看成一次策略决策:谁在请求、请求目的是什么、涉及哪些数据、是否包含隐藏策略或第三方内容、是否需要脱敏、是否需要审计。

例如普通用户通常需要知道结论依据、引用来源、已执行动作和下一步风险,但不需要看到完整系统提示词、其他用户消息、内部安全策略或长期记忆原文。工程师调试时需要 span、tool result、state diff 和版本信息,但也未必需要看到所有用户隐私字段。安全审计需要身份、权限、确认、audit id 和责任链,研究人员通常只应拿到脱敏样本和聚合失败类型。
...
| --- | --- |
| 结论和证据摘要 | 可给当前用户 |
| 引用和 artifact id | 可给当前用户,敏感来源需脱敏 |
| tool/event trace | 默认给工程或审计角色 |
| prompt/context 原文 | 高敏感,按需最小披露 |
| policy decision | 给工程和审计,用户侧只展示影响结果的部分 |
| memory refs | 需要 scope 和用户授权 |
| failure attribution | 工程可见,用户侧可摘要化 |

这样设计后,解释就不再是一段“越完整越好”的长文本,而是一个可裁剪的证据包。它既能支持事故复盘,也能避免把安全边界、个人隐私和第三方数据暴露给不该看到的人。
