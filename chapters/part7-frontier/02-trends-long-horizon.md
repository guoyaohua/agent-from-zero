...
长时程 Agent 不能只靠阶段总结防漂移。总结通常是自然语言,容易遗漏状态污染、权限扩大和预算空转。更稳的做法是维护一份机器可读的漂移账本,每个 checkpoint 都更新一次。

![长时程 Agent 漂移账本](../assets/part7-long-horizon-drift-ledger.svg)

漂移账本至少记录四类信号。

第一是目标信号。当前子任务是否仍服务 `goal_version` 和 `done_when`;如果用户新加了范围,它是目标变更、补充约束,还是临时插曲。没有这个字段,Agent 很容易把“顺手也做一下”当成新主线。

第二是状态信号。哪些事实是已验证的,哪些只是 hypothesis;哪些证据过期,哪些 artifact 被新版本替代;哪些失败工具调用不应该被写成完成状态。长任务的很多坏结果不是来自单次回答,而是来自错误状态被后续步骤继续消费。

第三是预算和进展信号。每次模型调用、检索、工具等待和人工确认是否换来了新证据、新产物或新决策。如果连续几轮只有成本增长,没有信息增量,就应该触发无进展停止。

第四是权限信号。长任务的授权通常按阶段给出,但风险会随任务演化。一个开始时只读的研究任务,后面可能想发送邮件、改配置或运行脚本。每次权限升级都应重新过 Harness 门,不能因为任务已经运行很久就默认放行。

这份账本的价值在于把“感觉有点跑偏”变成可检查条件:目标版本不匹配、旧证据继续使用、预算换不来进展、权限跨过原始范围。只要这些条件能被 runtime 看到,长时程 Agent 就能在出大错前停下来。
...
![长时程 Agent 健康账本](../assets/part7-long-horizon-health-ledger.svg)

上图把长时程 Agent 的运行状态看成健康账本。目标一致性回答“现在做的事是否仍服务原目标”;状态新鲜度回答“事实、偏好和证据是否过期”;进展速度回答“是否真的产生新证据、新产物和新决策”;预算消耗回答“成本是否换来了进展”;风险状态回答“权限、确认和回滚是否仍在可控范围”。

长任务最危险的不是慢,而是“看起来一直在忙,但已经偏离目标”。健康账本可以让系统定期自检:如果成本持续增加但没有新增证据,应触发无进展停止;如果旧证据过期,应重新验证;如果权限逐步扩大,应重新 review。长时程自主的核心能力,是持续证明自己还在正确轨道上。
...
| 阶段 | 验收问题 |
| --- | --- |
| 启动 | 目标、完成标准、预算、权限是否明确 |
| 探索 | 新证据是否足够,假设是否标注未验证 |
| 执行 | 每个子任务是否有产物、证据和状态更新 |
| 检查点 | 是否能从 checkpoint 恢复或回滚 |
| 交付 | 最终产物是否引用证据并说明剩余风险 |

如果某个阶段无法验收,就不要让 Agent 自动跨到下一阶段。长时程系统要把大目标拆成可检查的小闭环,每个小闭环都能停止、复盘、修正。这样长期自治才不会变成长期失控。
