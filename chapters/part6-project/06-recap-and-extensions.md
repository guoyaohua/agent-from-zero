...
扩展路线不应该只按“功能越来越多”排序,还要按控制面成熟度排序。每个新能力都应该带着权限、trace、eval、回滚和安全样本一起上线。

![研究助手扩展成熟度路线](../assets/part6-extension-maturity-roadmap.svg)

可以把扩展成熟度分成四个阶段。

第一阶段是单用户可信闭环。目标不是覆盖所有资料源,而是把 evidence pack、引用校验、笔记确认、trace 和小 eval set 做稳。这个阶段的控制面是最小但完整的:状态、预算、引用、确认和回归门禁。

第二阶段是检索质量增强。新增 hybrid search、reranker、chunk viewer、hard negatives 和检索评估。控制面要同步增加资料入库版本、权限过滤、source freshness 和召回/排序监控。

第三阶段是工作流增强。可以引入 Critic、多 Agent handoff、主动研究草稿和更复杂的冲突证据处理。控制面要补 handoff contract、trajectory eval、checkpoint、预算和 no-progress detection。

第四阶段是团队和生产化。新增多用户、组织资料源、写工具、审计、DLP、红队、集中 trace、限流、密钥管理和 SLO。控制面从“本地可信助手”升级为“有权限的软件系统”。这一步不能靠 prompt 补齐,必须靠 runtime、策略、运维和事故响应。

每次扩展都可以问一个简单问题:新能力有没有扩大不可控半径? 如果扩大了,对应的可解释半径是否也扩大了? 例如新增资料源扩大事实输入半径,就要新增权限和检索 eval;新增写工具扩大副作用半径,就要新增 preview/confirm/rollback;新增团队用户扩大信任边界,就要新增所有权、审计和删除传播。

扩展不是堆功能。好的扩展让系统能做更多事,同时让失败更容易定位、影响更容易限制、版本更容易回滚。
