...
需求冻结前,建议把每条用户故事写进一张风险账本。账本不是项目管理文档的装饰,而是后面架构、trace、eval 和发布门禁的输入。如果一条需求无法说清“最可能怎样错、在哪个 trace 字段里能看见、用哪类样本复现、发布前用什么条件拦住”,它就还不是工程需求。

![研究助手需求风险账本](../assets/part6-project-requirements-risk-ledger.svg)

风险账本可以按下面的粒度写:

| 用户故事 | 最容易发生的错 | 必须记录的信号 | 最小门禁 |
| --- | --- | --- | --- |
| 快速解释 | 引用主题相关但不支持具体结论 | `claim -> evidence_id -> citation_check` | 关键 claim 不支持则不能发布 |
| 方案比较 | 只召回支持单边观点的资料 | `query_plan`、`conflict_found`、`missing_dimension` | 对比维度和冲突证据必须显式呈现 |
| 阅读资料 | 把助手推断写成作者观点 | `quote`、`interpretation`、`summary_claim` | 摘要中的作者观点必须能回到原文 |
| 沉淀笔记 | 临时结论或外部注入污染长期记忆 | `note_draft`、`source_ids`、`user_confirmation` | 未确认写入为 0,来源缺失阻塞 |

这张账本会直接改变实现顺序。很多团队会先做漂亮回答,最后才补引用校验和评估;更稳的顺序是先定义失败模式,再决定 state 和 trace 里必须有哪些字段。比如“引用准确”不是一句 prompt,而意味着 answer 中的每个关键 `Claim` 都能映射到 evidence pack 中的 `quote`,并且 citation checker 能在发布门禁里独立拦截 unsupported claim。

需求账本也能防止过早扩展。只要某个新功能无法进入账本,例如“自动同步到团队知识库”说不清权限、确认、回滚和删除后的检索行为,它就不应该进入 v1。Agent 项目的边界不是靠意志力守住的,而是靠这些可验证的门禁守住的。

## 需求验收账本

风险账本回答“最可能怎样错”,验收账本回答“怎样证明它做对了”。对 Agent 项目来说,这两张账本要一起写。否则需求会停留在“支持某功能”,而不是落成可以实现、可以测试、可以回归的工程对象。

![研究助手需求验收账本](../assets/part6-requirements-acceptance-ledger.svg)

每条用户故事至少要映射到四类内容。

第一是系统对象。例如“快速解释”不是一段回答,而是 `Claim`、`Evidence`、`Answer` 和 `CitationReport`;“沉淀笔记”不是一个保存按钮,而是 `NoteDraft`、`Note`、`source_ids`、`confirmation_id` 和删除状态。

第二是 trace 信号。验收时必须能看到关键链路:query 如何生成、证据如何进入 evidence pack、claim 如何绑定引用、笔记写入是否经过确认。没有 trace 信号,功能即使跑通也无法复盘。

第三是 eval 样本。每个用户故事至少要有 happy path、hard negative 和边界样本。例如方案比较要测“只召回单边资料”的情况;阅读资料要测“把助手推断误写成作者观点”的情况。

第四是发布门禁。门禁应该写成可阻塞条件:伪引用为 0、关键 claim 不支持则不能发布、未确认笔记写入为 0、外部注入不能进入长期记忆。门禁越具体,后面架构和实现越不会跑偏。

需求验收账本的意义是把 v1 范围钉牢:不是少做功能,而是每做一个功能都能证明它可靠。这样读者照着实现时,不会被“再加一个资料源”“再加一个自动同步”带偏。
