...
核心循环不是“能一直跑”,而是每一轮都能证明自己更接近完成。研究助手尤其容易陷入三种空转:反复改写 query、反复读取相似 chunk、反复修正引用但没有新增证据。解决方式是为 Loop 维护一份收敛账本。

![研究助手循环收敛账本](../assets/part6-loop-convergence-ledger.svg)

收敛账本至少检查四件事。

第一,证据是否有增量。新一轮检索或阅读是否新增了能支持关键 claim 的 quote,还是只返回了重复、噪声或同一来源的改写。如果没有证据增量,下一步应该改 query、换资料源或停止,而不是继续读相似片段。

第二,状态是否有变化。`open_questions` 是否减少,`gaps` 是否被填补,`conflicts` 是否被解释,`plan` 是否推进。如果 state diff 连续两轮几乎相同,应触发 `no_progress`。

第三,预算是否换来信息。每次模型调用、检索、读取和 Judge 都要消耗预算。预算账本要能回答:这次消耗换来了新证据、新判断、新校验结果,还是只是在原地自我修正。

第四,安全边界是否仍满足。每轮工具调用和笔记写入都要重新过 Harness。不能因为前几轮都安全,就默认后续 action 也安全。

一个简单策略是给 task state 增加 `progress_markers`:

```json
{
  "new_evidence_count": 2,
  "open_questions_delta": -1,
  "unsupported_claims_delta": -2,
  "budget_spent_usd": 0.04,
  "no_progress_rounds": 0
}
```

当 `no_progress_rounds` 达到阈值时,Loop 应停止并输出当前结论、缺口和建议下一步。可靠循环的标志,不是它永不放弃,而是它知道什么时候继续会变成浪费或风险。
