...
```mermaid
flowchart LR
    A[前言与导读] --> B[Part 0 入门篇]
    B --> C[Part 1 模型与算法原理篇]
    C --> D[Part 2 Agent 核心原理篇]
    D --> E[Part 3 能力构建篇]
    E --> F[Part 4 架构进阶篇]
    F --> G[Part 5 工程实践篇]
    G --> H[Part 6 实战篇]
...
这张图是线性路径,但实际阅读可以来回跳转。比如你在 Part 6 做项目时遇到检索质量问题,可以回到 Part 3 的 RAG 和切块章节;遇到延迟问题,可以回到 Part 1 的 KV Cache、PagedAttention 和推理优化章节;遇到权限与安全问题,可以回到 Part 5。
...
Part 1 是本书最偏原理的一篇。它从向量、矩阵、概率直觉讲起,逐步进入神经网络、Transformer、训练对齐和推理优化。
...
- Flash Attention、PagedAttention 和投机解码解释了推理系统如何优化成本与延迟。

读完 Part 1 的主线,你应该能用直觉解释 LLM 如何处理输入、生成输出,以及为什么它会受上下文、采样、训练数据和推理机制影响。读完进阶章节后,你会更容易判断复杂 Agent 问题到底是模型问题、上下文问题、检索问题,还是系统设计问题。
...
Agent 面对真实任务时,往往需要处理大量外部知识和历史状态。只依赖模型参数是不够的,只依赖上下文窗口也不够。因此需要短期记忆、长期记忆、RAG、向量检索、工具使用进阶、反思和上下文工程。
...
这一篇会介绍为什么需要多 Agent、如何设计角色职责、常见编排模式、通信协议与消息传递,以及 MCP 这样的模型上下文协议。
...
Part 5 关注生产系统必须面对的问题:可观测性、评估、成本、延迟、安全、护栏和 Prompt Injection 防御。
...
| 构建能力 | Part 2 到 Part 4 | 能设计带工具、记忆、RAG 或多角色协作的 Agent |
| 走向生产 | Part 5 到 Part 7 | 能评估、调试、优化并扩展 Agent 系统 |

这四个阶段不是考试关卡。你可以边学边做,边做边回看。真正的掌握,往往发生在你发现系统失败、定位原因、做出改进的过程中。
