...
你可能刚开始接触 AI,知道 ChatGPT、Claude、Gemini、通义、DeepSeek 等模型,也尝试过让模型写代码、总结文档或回答问题。但你还不清楚 LLM 和 Agent 的关系,不知道 Prompt、Token、上下文窗口、RAG、工具调用这些词到底如何拼在一起。
...
你可能不满足于“调 Prompt 就行”,想知道模型为什么能生成语言,Self-Attention 到底在做什么,RLHF 和 DPO 为什么影响模型行为,KV Cache 为什么能加速推理,Flash Attention 为什么重要。

对你来说,Part 1 是重点。它会从数学直觉讲到 Transformer、训练、对齐和推理优化。目标不是把你训练成大模型研究员,而是让你能用原理解释工程现象。
...
数学部分不要求你提前掌握线性代数、概率论或深度学习。Part 1 会从向量、矩阵、概率直觉讲起。遇到公式时,重点不是背推导,而是理解符号背后的作用。
...
| 原理深潜 | 算法、研究、基础设施方向读者 | 精读 Part 1 的 `[进阶]` 与 `★` 章节 | 能用模型原理解释应用层现象 |
...
4. `part2`:掌握 Agent 循环、ReAct、工具调用和工作流选择。
5. `part3`:理解记忆、RAG 和上下文工程。
6. `part4`:先读多 Agent 与 MCP 的主线章节。
7. `part5`:学习可观测、评估、成本、安全与护栏。
8. `part6`:跟随项目把知识串起来。
9. `part7`:了解前沿趋势与开放问题。
...
1. 快速浏览 `part0`,补齐术语。
2. 阅读 `part2`,理解 Agent 的基本运行方式。
3. 精读 `part3`,尤其是 RAG、工具使用和上下文工程。
...
- 理解 KV Cache、Flash Attention、PagedAttention、投机解码等推理优化的意义。
