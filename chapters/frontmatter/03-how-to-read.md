...
`[进阶]` 面向希望深入理解机制和工程细节的读者。这类章节通常会回答“为什么”和“如何取舍”的问题,例如注意力变体、长上下文技术、多 Agent 编排、LLM-as-a-Judge、安全对齐等。

如果你第一次阅读时觉得吃力,可以先跳过。等你在项目中遇到相关问题,再回来读,往往更容易理解。

### `[参考]`

`[参考]` 更像速查资料或延伸阅读,不一定需要连续精读。比如术语表、模型全景、Prompt 速查等内容,适合在需要时回来查。
...
第一条是原理线:从数学直觉、神经网络、Transformer、训练对齐到推理优化,解释 LLM 能力从哪里来,限制又在哪里。

第二条是应用线:从 Agent 循环、ReAct、工具调用、RAG、记忆、上下文工程、多 Agent 到生产实践,解释如何把模型放进真实系统。

这两条线会互相照亮。比如理解 Self-Attention 后,你会更清楚上下文顺序为什么重要;理解 KV Cache 后,你会更清楚为什么长对话成本会上升;理解 RLHF 后,你会更清楚模型为什么倾向于给出“看起来有帮助”的回答,即使事实并不可靠。
...
如果能回答这四个问题,说明你已经不只是“知道一个名词”,而是在形成可用的技术直觉。
