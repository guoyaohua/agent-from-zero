...
![激活函数如何把线性边界弯起来](../assets/part1-activation-decision-boundary.svg)

更精确地说,线性层会改变空间坐标,激活函数会对空间做非线性折叠。多层网络反复执行“线性重组 + 非线性折叠”,就能把原本难以分开的样本变到更容易分开的表示空间。

这也是为什么“多层”本身不够,必须有非线性。没有激活函数时:

$$
W_3(W_2(W_1x))=(W_3W_2W_1)x
$$

它仍然只是一层线性变换。加入激活函数后,每一层的输出不再能简单合并,网络才真正获得层级表达能力。

从 LLM 角度看,激活函数还承担“选择性通过”的角色。不是所有中间特征都应该同等写入下一层。GELU、SwiGLU 等函数会让某些方向更强、某些方向更弱,使模型能根据当前 token 的上下文状态打开或关闭不同特征通道。模型越大,这种选择性越重要,否则所有信息都线性混在一起,表达会变得笨重且不稳定。
...
![Transformer FFN 的扩维、门控与回写](../assets/part1-transformer-ffn-expansion-gating.svg)

把 FFN 叫作“前馈网络”容易让人低估它。在 Transformer 层里,FFN 更像每个 token 的局部加工工作台。Attention 已经把其他 token 的信息混入当前位置,FFN 则在当前位置内部判断:哪些组合特征应该增强,哪些应该压低,哪些应该写回 residual stream。

现代 LLM 的 FFN 往往不是简单的 $W_2\sigma(W_1x)$,而是带门控的结构。以 SwiGLU 风格为例,可以粗略写成:

$$
\text{FFN}(x)=W_{down}\left(\text{SiLU}(W_{gate}x) \odot W_{up}x\right)
$$

其中 $\odot$ 表示逐元素相乘。$W_{up}$ 负责把向量扩展到更宽的中间空间,$W_{gate}$ 产生一个门控信号,决定哪些中间特征通过,$W_{down}$ 再把结果投影回模型主干维度。

这个结构有两个重要含义。

第一,扩维提供容量。中间维度 $d_{ff}$ 通常显著大于 $d_{model}$,让模型有更大的临时空间组合特征。就像把一份压缩表示展开到更宽的工作台上处理,处理完再写回主通道。

第二,门控提供条件计算。当前 token 表示不同,门控打开的特征通道也不同。同一个 FFN 参数可以在不同上下文下表现出不同的加工路径,这对语言中的多义性、格式切换、代码语法和工具调用模式都很重要。

因此,Attention 不是 Transformer 的全部。Attention 决定信息从哪里来,FFN 决定这些信息在当前位置如何被解释、改写和压缩回主表示。
