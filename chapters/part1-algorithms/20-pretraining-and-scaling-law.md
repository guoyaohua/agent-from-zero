...
![从预训练到 Agent 可靠性的能力栈](../assets/part1-pretraining-agent-capability-stack.svg)

上图把“预训练很强”和“Agent 很可靠”之间缺的层次摊开来看。预训练带来语言、代码、知识和模式识别能力;SFT、偏好优化和工具轨迹数据让模型更像助手;Harness 和 Loop 把工具、状态、权限、预算和恢复机制接起来;评估与安全层负责发现退化、控制风险和沉淀事故经验。

因此,Scaling Law 解释的是地基变强的趋势,不是完整产品可靠性的保证。一个 Agent 出错时,根因可能在基础模型能力不足,也可能在上下文组织、工具契约、状态压缩、权限设计或回归评估缺失。把这些层分开,才能避免把所有问题都误归因成“模型还不够大”。
