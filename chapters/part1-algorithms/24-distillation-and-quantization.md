...
![压缩模型部署路由](../assets/part1-compression-deployment-router.svg)

压缩模型上线时,最稳的方式通常不是直接替换强模型,而是引入路由。高频、低风险、可程序校验的任务交给蒸馏或量化模型;低置信、高风险、长尾复杂任务回退强模型;最后再用 schema、安全规则或引用校验做验证。这样压缩模型承担它擅长的部分,而不是被迫处理所有边界。

这套模式的收益取决于流量结构。如果 80% 请求是稳定分类、抽取、路由和格式转换,小模型能显著降本。如果多数请求是复杂推理、长上下文代码和高风险工具操作,压缩收益就会被回退和错误成本抵消。压缩前先看 trace 中的任务分布,比盲目追求 INT4 更重要。

![压缩模型上线风险门](../assets/part1-compression-rollout-risk-gates.svg)

上图把压缩模型上线拆成一组风险门。第一步不是问“小模型省了多少显存”,而是用强模型 trace 建 baseline,再让蒸馏或量化模型在同一批目标任务上对比质量、格式合法率、工具成功率和安全边界。离线通过后,还应该先跑 shadow traffic,观察它在真实输入分布下会怎么回答,再进入小流量灰度。

压缩真正可上线的标志是失败可发现、可路由、可回滚。比如小模型在低风险分类上表现稳定,但在长上下文工具调用中错误率上升,就应该让路由器把这类请求回退强模型。压缩是成本优化,不是能力升级;只有当净收益扣除回退、重试、人工复核和事故风险后仍然成立,它才是工程上划算的选择。

## 压缩模型的线上监控

压缩模型上线后,不能只监控 GPU 利用率和平均延迟。更重要的是观察它有没有把错误转移到下游系统。

建议至少监控这些指标:

| 指标 | 为什么重要 | 触发动作 |
| --- | --- | --- |
| 回退率 | 小模型无法处理的比例 | 回退率升高说明任务分布变化或置信策略太严 |
| 格式错误率 | 结构化输出是否还能解析 | 自动重试、回退强模型、修工具 schema |
| 工具失败率 | 参数和权限是否退化 | 检查蒸馏轨迹和参数来源 |
| 人工复核失败率 | 压缩是否影响真实质量 | 暂停灰度或缩小适用范围 |
| 安全拦截率 | 拒绝和确认是否异常 | 检查边界样本是否被压缩破坏 |

压缩模型最危险的失败不是“明显变笨”,而是平均表现还行,但长尾边界变差。比如 INT4 后普通客服回答没问题,但退款、账号权限、工具 JSON、长文档引用开始不稳定。上线策略应该让压缩模型先服务低风险、可验证、高频任务,再逐步扩大范围。
