...
![长上下文证据提升循环](../assets/part1-long-context-evidence-promotion-loop.svg)

上图把长上下文 Agent 的上下文组织拆成一个循环:先从资料库中检索和重排,再把证据编号成 evidence index,然后放入分区上下文。到关键决策前,系统再把最相关证据提升成短 evidence pack,放到靠近当前指令的位置,最后在回答中保留引用和回源信息。

这种做法承认一个现实:模型能“看到”长上下文,不代表它会在关键时刻稳定使用中间位置的材料。证据提升不是简单重复,而是在长上下文的工作台上给关键材料打灯。它也让评估更具体:如果回答错了,可以追踪是检索没召回、证据索引丢了、提升选错了,还是模型在有证据时仍然推理错误。

## 长上下文构造模板

一个可复用的长上下文模板,通常不是“资料全文 + 用户问题”,而是分区后的工作包:
...
这个模板的重点是把“索引”和“细节”分开。索引帮助模型知道材料在哪里,细节提供推理依据,提升证据帮助模型在当前位置使用关键事实。长上下文越长,越不应该依赖自然顺序;要依赖编号、标题、来源、边界和回源路径。
