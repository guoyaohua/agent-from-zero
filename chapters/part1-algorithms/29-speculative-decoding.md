...
![投机解码端到端瓶颈诊断](../assets/part1-speculative-decoding-trace-bottleneck.svg)

上图强调投机解码的第一步不是选草稿模型,而是拆端到端 trace。如果一次 Agent 调用的主要时间花在工具等待、网络、排队或超长 prompt prefill 上,decode 再快也只会改善一小段。只有当 decode token 时间占比高、输出足够长、草稿模型足够便宜且 acceptance rate 稳定时,投机解码才可能明显改善 p95 延迟。

上线评估也不能只看“平均快了多少”。要按任务类型分别看 acceptance rate、TPOT、端到端 p95、质量回归和资源占用。固定格式生成和局部代码补全可能收益很好,但高温创作、长推理解释、参数依赖强的工具 JSON 可能接受率波动。投机解码解决的是服务端 decode 串行瓶颈,不是 Agent 系统所有等待的总开关。

## 如何粗算投机解码收益

可以先用一个简化模型估算是否值得做实验。设草稿长度为 $k$,平均接受草稿 token 数为 $a$,目标模型一次验证的成本约等于一次 decode 前向的若干倍,草稿模型生成 $k$ 个 token 的成本为 $C_d$,目标模型验证成本为 $C_t$。投机解码一轮平均推进约 $a+1$ 个 token,粗略单位 token 成本可以写成:

$$
\text{cost per token} \approx \frac{C_d + C_t}{a + 1}
$$

这个式子不用于精确预测,但能帮助你看清三个杠杆:草稿要便宜,$a$ 要高,目标验证不能太贵。如果 $a$ 很低,分母接近 $1$,你只是额外跑了草稿模型;如果草稿模型太大,$C_d$ 会吃掉收益;如果目标验证实现不能高效并行检查草稿 token,$C_t$ 也会变重。

真实系统还要把 prefill、排队、采样、KV Cache 管理和网络开销放进端到端 trace。一个常见上线流程是:先在离线日志上重放不同任务类型,统计 acceptance rate;再在影子流量上测 TPOT 和 GPU 利用率;最后小流量灰度看 p95/p99 延迟和质量回归。没有这些分层数据,平均速度提升很容易骗人。
...
下一章会继续看测试时计算与推理型模型。投机解码关注“同样输出能不能更快生成”,测试时计算则关注“复杂任务是否值得在回答阶段投入更多搜索、验证和修正预算”。再往后,我们会用 MoE 收束模型架构和推理服务之间的取舍。
