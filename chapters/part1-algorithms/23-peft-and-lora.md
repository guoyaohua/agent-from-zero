...
![LoRA 适配器发布治理](../assets/part1-lora-adapter-release-governance.svg)

上图把 LoRA 发布拆成五个对象:数据版本、适配器配置、基础模型版本、评估门禁和发布回滚。LoRA 的可插拔性让实验更快,也带来组合爆炸。一个 adapter 在 `base:v1` 上通过,不代表它在 `base:v2` 上仍然安全;一个 rank 在离线集上好,不代表线上工具轨迹不会变激进。

因此 LoRA 适配器要像独立制品一样治理。每个版本应记录训练数据桶、清洗规则、rank、alpha、target modules、基础模型哈希、tokenizer、评估集结果和灰度记录。对 Agent 来说,尤其要评估轨迹指标:是否过度调用工具、是否在失败后恢复、是否在高风险动作前确认,而不只是最终回答是否更像公司风格。

## LoRA 发布清单

发布 LoRA 前,可以用一个简短清单检查风险:

| 检查项 | 要问的问题 |
| --- | --- |
| 数据版本 | 样本是否有来源、过滤规则和隐私检查 |
| 组合版本 | base、adapter、tokenizer 是否固定 |
| 目标能力 | 目标集是否显著提升 |
| 回归能力 | 通用问答、代码、安全是否退化 |
| 工具轨迹 | 工具选择、参数、失败恢复是否稳定 |
| 发布策略 | 是否支持灰度、回滚和多版本隔离 |

如果 LoRA 用于多租户场景,还要考虑适配器隔离和加载策略。动态加载方便切换,但要防止错配租户、错配基础模型或缓存污染。合并权重路径更简单,但回滚和多任务切换成本更高。选择哪一种,取决于产品是否需要频繁切换适配器。
