...
![RNN 的长距离依赖和状态瓶颈](../assets/part1-rnn-gradient-bottleneck.svg)

这张图把 RNN 的两个困难放在一起看:前向信息要沿隐藏状态链传递,反向梯度也要沿同一条链传回去。链越长,每一步状态更新都可能改变早期信息,每一步反向传播也都可能缩小或放大梯度。

更形式化地说,如果后面损失 $L$ 想影响很早的状态 $h_1$,梯度里会出现很多局部导数的乘积:
...
这些矩阵或标量反复相乘后,范数可能越来越小,也可能越来越大。这就是梯度消失和爆炸的根源。LSTM、GRU、梯度裁剪、初始化和归一化都可以缓解,但不能改变 RNN 必须按时间链传递信息的基本结构。

这和 Agent 工程中的“摘要记忆”很像。把几十轮对话压成一段短摘要后,摘要确实便宜,但早期细节很容易丢失或被新内容覆盖。压缩状态越短,越需要明确写入不可丢失的约束、事实来源和待办项。
...
![LSTM 用门控保护长期记忆通道](../assets/part1-lstm-gates-cell-state.svg)

可以把 LSTM 看成给普通 RNN 加了一条更稳定的记忆高速路。普通 RNN 每一步都把隐藏状态整体重写一次;LSTM 则把长期信息放进 $c_t$,再用门控决定这条通道上每个维度如何变化。

遗忘门不是简单“忘掉旧信息”,而是学习哪些维度应该继续保留。输入门不是无条件写入新内容,而是学习当前输入哪些部分值得进入长期记忆。输出门则把细胞状态中对当前预测有用的部分暴露成 $h_t$。

这种设计带来的好处是,某些重要信号可以跨越多个时间步保持相对稳定。例如主语、实体、括号状态、语法模式等,都可能通过细胞状态持续影响后续输出。

但 LSTM 的局限也要看清:它仍然要把历史压进状态,仍然要按时间步递归计算。它改善了“记忆怎么保留”,但没有变成“当前位置可以直接查看任意历史位置”。这正是 Attention 后来进一步突破的地方。
