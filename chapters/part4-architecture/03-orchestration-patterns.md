...
![Blackboard 治理泳道](../assets/part4-blackboard-governance-lanes.svg)

这张图强调:黑板不是一块自由文本白板,而是按目标、计划、证据、假设、冲突和决策分区的共享状态库。每个分区的写入规则不同,否则“大家都能贡献”很快会变成“没人知道该信什么”。
...
黑板最好按数据类型分区,而不是只有一个大文本区。目标、计划、证据、假设、冲突和决策应该有不同 schema 和状态机。例如证据可以是 `candidate -> verified -> rejected`;假设可以是 `unverified -> supported -> contradicted -> superseded`;决策必须引用证据和仲裁规则。
...
这样做会稍微增加实现成本,但能避免黑板从“共享状态”退化成“共享幻觉”。在复杂任务里,真正宝贵的不是所有角色都能写,而是每个角色写入后,系统知道这条内容是什么类型、处于什么状态、谁负责、是否被采纳、被什么取代。
...
多 Agent 编排里,最难排查的错误之一是“为什么系统突然调用了这个角色”。如果 Orchestrator 的决策只存在于模型的隐式推理中,后续很难复盘。调度本身应该像工具调用一样有账本。

![编排调度账本](../assets/part4-orchestration-scheduling-ledger.svg)

一次调度决策至少记录:

| 字段 | 作用 |
| --- | --- |
| `trigger` | 哪个事件触发调度:新消息、工具失败、证据缺口、超时 |
| `state_snapshot_id` | 调度前的权威状态版本 |
| `candidates` | 哪些角色理论上可以处理 |
| `chosen_role` | 最终选择哪个角色 |
| `why_chosen` | 选择原因,例如“关闭 missing_evidence gap” |
| `context_pack_id` | 给该角色看的上下文包 |
| `budget_before` | 剩余 token、时间、工具调用次数 |
| `permission_decision` | 该角色本轮允许哪些工具和数据 |
| `stop_rule` | 这一步何时停止或升级 |

调度账本能发现两类常见问题。第一类是过度调用:系统在证据已经充分时仍反复让 Researcher 搜索,说明停止条件或进展判断有问题。第二类是错误路由:工具权限错误却交给 Critic 评审,而不是回到 Executor 或权限门处理。

如果 Orchestrator 里有 LLM 参与路由,账本更重要。可以让 LLM 给出候选角色和理由,但最终调度仍经过确定性检查:预算是否够、角色是否有权限、上下文包是否可构建、是否命中停止条件。这样 LLM 的灵活性不会绕过 runtime 的安全和恢复机制。
