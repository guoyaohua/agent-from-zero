...
## 工具权限矩阵:角色边界必须由 runtime 执行

角色设计如果只停留在 prompt 里,很容易失效。模型可以被上下文诱导,也可能在任务压力下越过“请不要做”的软约束。真正的角色边界应该落到 runtime 的工具权限矩阵里。

![角色工具权限矩阵](../assets/part4-role-tool-permission-matrix.svg)

权限矩阵要区分几类能力:

| 能力 | 风险 | 设计建议 |
| --- | --- | --- |
| 读资源 | 泄露、越权、上下文污染 | 按用户、项目、角色过滤 |
| 检索 | 召回不可信或无权限资料 | 来源白名单、证据标注 |
| 执行写工具 | 修改外部状态 | preview、confirm、idempotency |
| 发布/提交 | 对用户或业务产生真实影响 | 高风险确认、审批、审计 |
| 阻塞动作 | 影响流程推进 | Critic 可阻塞,但要给 reason code |

例如 Researcher 可以读授权文档和检索资料,但不应拥有退款提交工具。Executor 可以调用订单查询和草稿创建工具,但最终发布可能仍需用户确认或 Coordinator 放行。Critic 可以阻塞高风险动作,但不应该直接代替 Executor 调写工具。这样一来,即使某个角色被错误提示诱导,它也没有足够权限造成越界副作用。

权限矩阵最好和角色契约一起版本化。每次新增工具、扩大资源范围或调整角色职责,都应该触发评审:这个工具是否真的需要给该角色? 输出是否包含敏感数据? 结果是否会通过消息传给拥有写权限的角色? 多 Agent 的安全不只在单个工具调用点,也在角色之间的数据流里。
...
![角色交接契约](../assets/part4-role-handoff-contract.svg)

交接契约的重点是让下游“少猜”。下游角色不应该重新推断上游到底完成了什么、证据在哪里、哪些限制仍然有效。一个可用的 handoff 至少包含:

| 字段 | 作用 |
| --- | --- |
| `task_goal` | 防止下游偏离原始目标 |
| `deliverable` | 说明上游产物是什么版本的 schema |
| `evidence_refs` | 让结论能回到来源、版本和片段 |
| `gaps` | 显式保留“不知道什么” |
| `constraints` | 传递禁止动作、用户确认和安全边界 |
| `artifact_refs` | 大对象不塞消息,只传可回源引用 |
| `trace_refs` | 出错时能回放上游过程 |

最容易丢的是 `gaps` 和 `constraints`。Researcher 可能在自然语言里写“没有找到审批记录”,下游 Synthesizer 转述时变成“审批记录缺失”,再下一轮就被误读为“没有审批”。这就是消息退化。把缺口写成结构化字段,并标注它对结论的影响,可以防止不确定性在交接中被悄悄升级为事实。

交接失败也要有明确返回路径。如果 Critic 收到的 `evidence_refs` 不存在,或 `constraints` 缺少高风险动作说明,它不应该继续猜,而应该返回 `error.v1` 或 `needs_more_input`。角色交接不是礼貌流程,而是多 Agent 的接口契约。
