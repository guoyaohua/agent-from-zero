...
> **规范快照**:本章按 **A2A `v1.0.1`（2026-05-28 发布）** 复核。当前规范定义 Agent Card、Message、Task、Artifact、流式与推送更新,并提供 JSON-RPC、gRPC、HTTP+JSON/REST 等协议绑定。实现细节应以双方声明的版本和绑定为准。
...
公开 Agent Card 还应遵守最小披露原则。能力描述要足以让调用方判断是否适配,但不应泄露内部主机名、敏感工具、租户信息或可被攻击者利用的实现细节。需要认证后才能公开的能力,可以通过扩展 Agent Card 或受保护发现流程提供。
...
版本兼容不能只看“HTTP 请求成功”。调用方还要验证协议版本、传输绑定、扩展 URI、输入输出模式和任务更新机制。未知扩展默认不启用;版本升级先经过 contract test 和可取消的影子任务,再进入真实委托。
...
A2A 的多协议绑定和扩展机制提高了互操作空间,也扩大了兼容性测试面。生产系统应把精确规范版本、绑定、Agent Card 快照、认证配置和扩展列表放进发布单元,而不是只记录一个 Agent URL。
