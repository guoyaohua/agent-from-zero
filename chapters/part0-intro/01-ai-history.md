...
![AI 范式从规则到 Agent 的迁移](../assets/part0-ai-paradigm-shifts.svg)

这张图要强调的是“智能来源”的变化。早期系统的智能主要来自人写规则;传统机器学习把部分判断交给数据;深度学习让表示也能从数据中学;基础模型把大量语言、代码和世界知识压缩进可迁移的参数;Agent 则把模型放回一个有工具、有状态、有反馈的环境里。

这条线并不是“后者消灭前者”。真实系统里,规则仍然适合权限和确定性流程,传统模型仍然适合稳定预测,检索仍然适合事实更新,LLM 适合语言和开放判断,Agent 则适合把这些组件组织成任务循环。

所以理解 AI 史的关键不是记住哪个阶段更先进,而是知道每种范式解决了什么、留下了什么缺口。Agent 的工程复杂性,正来自它需要同时继承这些能力和边界。
...
![现代 AI 系统是混合系统](../assets/part0-ai-hybrid-system.svg)

这也是为什么本书后面不会只讲 Prompt。Prompt 能影响模型行为,但不能替代权限系统;RAG 能补事实,但不能替代答案校验;工具调用能扩展能力,但不能替代参数检查和审计;评估能发现退化,但不能替代运行时护栏。

一个成熟 Agent 更像“多种 AI 和软件工程机制的组合”,而不是“一个特别聪明的模型”。当你设计系统时,要先判断每个环节需要什么性质:如果需要确定性,优先用规则和程序;如果需要最新事实,优先检索或查库;如果需要语言理解和模糊判断,再交给 LLM;如果动作有副作用,必须交给权限和确认机制把关。
