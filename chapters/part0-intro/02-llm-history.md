...
![下一个 token 预测如何逼出多种能力](../assets/part0-llm-next-token-capability.svg)

这里最容易误解的是:语言模型并不是在训练目标里直接写着“学会推理”“学会写代码”“学会总结”。训练目标只是提高真实下一个 token 的概率。但训练数据里包含论文、代码、问答、教程、日志、网页、数学推导、API 文档和人类对话。要在这些文本上持续降低损失,模型就必须压缩其中反复出现的结构。

例如,要预测代码补全,模型需要学语法、变量作用域和常见库调用;要预测教程段落,模型需要学解释结构;要预测问答文本,模型需要学问题与答案之间的对应;要预测推导步骤,模型需要学符号关系和局部逻辑。

所以“大模型能力”可以理解成一种副产品:单一概率目标在足够大、足够多样的数据和模型容量下,逼出了许多可迁移的内部表示。当然,这也解释了它的局限。模型学到的是数据分布中的模式,不是自动获得事实校验、现实反馈和行动权限。
...
![从预训练到 Agent 能力的对齐栈](../assets/part0-llm-training-alignment-stack.svg)

从 LLM 到 Agent,中间不是一步跳跃。预训练模型提供的是通用语言和知识底座,但它更像强大的续写器。指令微调让它学会按人类任务格式回答,偏好对齐让它更符合“有帮助、诚实、安全”的行为期待,工具协议让它能用结构化方式提出外部动作,系统护栏和评估再限制它能做什么、如何验证。

这条栈解释了一个重要工程事实:当 Agent 出错时,原因可能不在同一层。它可能是模型底座能力不足,也可能是指令数据没有覆盖该任务,可能是工具 schema 描述不清,也可能是权限边界或评估缺失。排查 Agent 问题时,不能只改 Prompt,还要定位是哪一层给了错误信号。
