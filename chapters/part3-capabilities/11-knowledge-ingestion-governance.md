...
![RAG 入库发布门禁](../assets/part3-ingestion-release-gates.svg)
...
知识入库也应该像代码发布一样有门禁。一次索引构建完成后,不应直接切到线上,而应经过 staged index、评估、灰度和回滚点。

| 门禁 | 检查内容 | 失败时动作 |
| --- | --- | --- |
| 解析门 | 标题、表格、代码、页码、锚点是否保真 | 修 parser 或标记不可入库 |
| 元数据门 | 来源、版本、生效时间、ACL、trust level 是否齐全 | 阻止发布,补元数据 |
| 检索门 | 标准问题能否召回正确 chunk | 调 chunking、embedding、BM25 或 reranker |
| 权限门 | 无权用户是否完全不可见 | 阻止发布,修 ACL 过滤 |
| 删除门 | 删除、下线、权限变化是否传播到索引和缓存 | 阻止发布,修删除传播 |
| 回归门 | 关键 RAG 评估集是否不退化 | 灰度或回滚 |

门禁要尽量自动化,但也要保留人工抽检。尤其是制度、合同、财务、HR、客户数据这类高风险知识源,一次解析错误或权限遗漏就可能让模型生成非常自信的错误答案。

## 删除传播的衍生物清单

删除传播不只删除向量库里的 chunk。生产 RAG 通常还有很多衍生物:

| 衍生物 | 为什么要处理 |
| --- | --- |
| embedding 向量 | 否则仍可能被召回 |
| BM25 倒排索引 | 关键词检索仍会命中旧内容 |
| reranker 缓存 | 旧候选排序结果可能被复用 |
| evidence pack 缓存 | 已打包证据可能越过新权限 |
| 摘要和压缩片段 | 摘要可能保留被删除事实 |
| 临时/长期图谱边 | GraphRAG 可能继续走旧关系 |
| 失败样本和 trace | 日志需脱敏或按保留期处理 |
| 长期记忆引用 | 记忆里不能保留已撤回知识结论 |

如果做不到完全物理删除,至少要有 tombstone 和查询时过滤,并能证明被撤回内容不会进入新的 context pack。对合规要求更高的场景,还需要记录删除请求、传播状态和完成时间。
...
到这里,Part 3 完成了 Agent 能力构建的主线:记忆、RAG、检索质量、工具使用、自我修正、上下文工程、Agentic RAG 和知识治理。下一篇 Part 4 会进入架构进阶:从单 Agent 走向多 Agent、角色分工、编排、通信协议和 MCP。多个 Agent 可以协作,但不能绕过资料权限、来源和生命周期。
