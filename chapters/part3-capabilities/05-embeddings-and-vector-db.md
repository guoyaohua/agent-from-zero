...
![向量相似度分数的诊断含义](../assets/part3-vector-score-diagnostics.svg)

这张图要拆掉一个常见误解:embedding 分数不是事实置信度。`score=0.82` 只能说明这个片段在当前模型、当前索引、当前度量和当前候选集合里比较相似,不能说明它能回答问题,更不能说明结论有 82% 可信。

把候选变成证据,还要继续检查来源权威性、版本状态、权限范围、是否只是主题相似、reranker 是否支持、能否支撑具体 claim。真正的置信度来自证据链,不是来自单个向量分数。
