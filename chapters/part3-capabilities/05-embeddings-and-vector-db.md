...
Embedding 把文本映射到向量空间,向量数据库负责高效存储和搜索这些向量。但检索质量不只取决于向量库,还取决于 embedding 模型、输入格式、切块、元数据过滤、索引参数、混合检索和重排。向量相似只是候选生成,不是事实正确。可靠 RAG 需要把向量检索当成可评估、可版本管理、可权限控制的数据系统。
