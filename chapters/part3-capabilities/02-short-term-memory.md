...
![工具结果进入短期记忆的三层结构](../assets/part3-short-term-observation-layers.svg)

这张图把工具结果拆成三层。第一层是原始 artifact,保证以后可以回源;第二层是结构化提取,用于状态更新和程序化校验;第三层是工作摘要,进入下一轮 context pack。三层缺一不可:只存原始结果会撑爆上下文,只存摘要会丢失复查能力,只存结构化字段又可能失去人类可读证据。

短期记忆的好坏,往往取决于这层转换是否可靠。工具返回“测试失败”不应该直接变成“代码 bug 已确认”,而应该先记录 `status=failed`、失败文件、断言、栈位置和 artifact,再把“当前假设”作为 hypothesis 写入 state。观察和推断分开,Agent 才不容易把猜测当事实。
