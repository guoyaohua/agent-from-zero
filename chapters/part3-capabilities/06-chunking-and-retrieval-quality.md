...
| 单元 | 用途 | 典型大小 | 设计重点 |
| --- | --- | --- | --- |
| 检索单元 | 被 embedding 和关键词检索命中 | 较小 | 主题单一,能匹配用户 query |
| 上下文单元 | 放进证据包给模型阅读 | 中等 | 条件完整,包含必要前后文 |
| 引用单元 | 支撑最终答案中的某个 claim | 精确 | 能被人核验,来源清楚 |

很多系统失败,是因为它用同一个 chunk 同时承担三件事:既要精准召回,又要完整阅读,还要精确引用。现实里更稳的做法是把它们分开。例如用小的子块做检索,命中后回源到父段落作为上下文,最终引用到具体条款或表格行。

这样读者能理解一个关键点:chunk 不是文档被切出来后的静态碎片,而是服务检索、阅读和引用的一套数据结构。

![检索单元、上下文单元和引用单元](../assets/part3-chunk-units-map.svg)

这张图把切块的核心取舍画出来:检索单元要小而聚焦,方便 query 命中;上下文单元要完整,让模型能读懂条件和例外;引用单元要精确,方便人和系统核验某个 claim。很多系统用同一个 chunk 同时承担三件事,结果就是小了无法回答,大了召回不准,引用又不精确。

父子块和邻接扩展的意义就在这里。可以用子块做召回,命中后回源到父段落或相邻条款作为上下文,最终引用到具体条款、表格行或代码行号。这样 RAG 才能同时兼顾命中率、可读性和可校验性。
...
切块策略一改,整个索引都会变。不要把切块调整当成无风险重构。

一个切块回归测试至少要覆盖:

| 测试 | 检查什么 |
| --- | --- |
| boundary test | 关键定义、条件、例外是否被切断 |
| recall test | 标注 query 是否仍能命中正确单元 |
| hard negative test | 相似但错误的块是否被压下去 |
| citation test | 最终引用是否能定位到足够精确的来源 |
| compression test | 组包压缩后是否丢失限制条件 |
| ACL test | 权限和版本过滤是否仍然正确 |

这类测试不需要一开始很大。几十个高价值 query 往往就能暴露大部分切块问题。重点是每次修改清洗规则、标题解析、chunk size、overlap、父子块关系时,都能看到质量是变好还是变坏。

## Chunk 可观测性

切块策略需要可观测,否则很难调。

![Chunk Viewer 调试台](../assets/part3-chunk-viewer-debug.svg)
...
一个好的 chunk viewer 不只是展示文本,还要能回答“为什么这次检索会这样”。例如点开某个 query 后,你应该能看到:原始文档如何被清洗,chunk 边界如何生成,命中的 chunk 之前和之后是什么,哪些元数据过滤掉了候选,reranker 为什么改变排序,最终证据包是否丢掉了例外条件。

当工程师能在一个界面里看到“原文 -> chunk -> 命中 -> 证据包 -> 答案引用”,切块优化才会从猜测变成调试。
...
### 误解六:一个 chunk 可以同时适合召回、阅读和引用

通常不行。检索单元、上下文单元和引用单元的目标不同,复杂系统最好显式区分。

### 误解七:切块策略只要上线前调好一次

不是。文档结构、用户问题、模型、索引和业务规则都会变化。切块需要回归测试和可观测工具持续维护。

## 本章小结

切块决定 RAG 的检索单元,是检索质量的上限之一。好的 chunk 应语义完整、主题单一、保留结构和来源,同时长度适合检索和上下文。固定长度切块简单但脆弱,结构化切块更适合政策、代码、网页和表格。父子块、邻接扩展、元数据过滤、hard negative 评估、chunk viewer 和切块回归测试能显著改善质量。诊断 RAG 时,要区分找不到、排不前和用不好三类问题。
