...
RAG 不是一种固定架构,而是一组围绕证据获取和证据使用的设计。不同 RAG 形态适合不同问题。

| 形态 | 核心做法 | 适合场景 | 主要风险 |
| --- | --- | --- | --- |
| Naive RAG | 一次 query、top-k 召回、直接生成 | 简单事实问答、低风险内部 FAQ | 证据不足仍强答、引用不忠实 |
| Keyword RAG | BM25/关键词召回 | 错误码、条款号、函数名、产品型号 | 近义表达和隐含语义召回差 |
| Vector RAG | embedding 语义召回 | 表达不同但语义相近的问题 | 精确实体、编号和否定条件不稳 |
| Hybrid RAG | 关键词 + 向量融合 | 生产知识问答的默认强基线 | 融合权重、去重和排序复杂 |
| Rerank RAG | 初召回后用 reranker 精排 | top-k 噪声较多、候选需要细粒度排序 | 成本更高,召回阶段漏掉的证据无法补救 |
| Multi-vector RAG | 一个文档块有多个向量或 late interaction | 长段落、表格、细粒度匹配 | 索引和 serving 成本更高 |
| Long-context RAG | 召回更多证据后放入长上下文 | 长报告、代码库、跨段综合 | 容量变大不等于选择正确,成本高 |
| GraphRAG | 抽取实体、关系、社区或证据图谱 | 多跳关系、全局主题、实体网络 | 抽取错误、关系过期、边不可回源 |
| Agentic RAG | 多轮检索、缺口管理、验证和停止 | 跨文档、跨版本、冲突证据问题 | 没有停止条件会越搜越乱 |

可以把选择顺序设成一个保守默认:

1. 先做 Hybrid RAG + reranker,建立可评估基线。
2. 如果问题需要多跳实体关系,再引入 GraphRAG 或 evidence graph。
3. 如果一次检索经常证据不足,再引入 Agentic RAG 的缺口循环。
4. 如果只是上下文塞不下,再评估长上下文和上下文压缩,不要直接把所有文档塞进去。

这能避免“看到新名词就换架构”。RAG 迭代应该由失败归因驱动:召回漏了,修召回;排序错了,修 rerank;证据包缺条件,修上下文组织;答案不忠实,修生成约束和引用校验。
