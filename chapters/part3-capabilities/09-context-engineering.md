...
![Context Builder 的质量门](../assets/part3-context-builder-gates.svg)

这张图把 context builder 看成一个带质量门的信息编排器。它从短期状态、长期记忆、RAG 证据、工具观察和工具 schema 中选择材料,再检查完整性、相关性、层级性、可追溯性、预算和安全标注,最后生成 context pack。

一旦这样看,上下文问题就能被测试。模型忘记约束,可能是完整性门漏了;模型被网页指令带偏,可能是层级性门没标注不可信内容;成本暴涨,可能是预算门没有裁工具 schema 或日志。上下文工程从“调 prompt 手感”变成了可回归的数据构建问题。
...
下一章会讲 Agentic RAG 与知识图谱。上下文工程回答“哪些材料进入本轮模型调用”,Agentic RAG 进一步回答“证据不足时,系统如何多轮检索、对比、验证并知道何时停”。
