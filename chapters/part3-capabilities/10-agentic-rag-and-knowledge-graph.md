...
![RAG 策略选择矩阵](../assets/part3-rag-strategy-selection-matrix.svg)
...
不要一开始就把所有 RAG 做成多轮 Agent。多轮意味着更多延迟、更多成本、更复杂的安全边界和更多失败路径。更稳的做法是先建立 Hybrid RAG + reranker + 引用校验基线,再根据失败样本决定是否升级。

| 失败信号 | 更可能需要 | 不应误用 |
| --- | --- | --- |
| 一次检索经常漏掉旧版本或补充规则 | Agentic RAG 的缺口循环 | 盲目增大 top-k |
| 问题依赖实体关系、上下游或社区归纳 | GraphRAG / evidence graph | 把所有文档都抽成永久图谱 |
| 精确编号、错误码、函数名召回不稳 | BM25 + hybrid fusion | 只换 embedding 模型 |
| 正确证据召回了但排序靠后 | reranker / hard negative | 增加生成模型规模 |
| 证据包丢失限制条件或例外 | context pack / compressor 规则 | 继续扩大检索范围 |
| 答案不忠于证据 | claim check / citation verification | 继续调 ANN 参数 |

这张表的价值是把“新架构冲动”转成失败归因。Agentic RAG 是为了解决证据缺口推进问题,不是为了让系统看起来更智能。GraphRAG 是为了解决关系和全局结构问题,不是替代所有向量检索。长上下文是容量工具,不是证据选择工具。

一个实用判断是:如果失败样本可以通过更好的切块、混合检索或 reranker 修好,先不要上 Agentic RAG;如果失败样本需要“先查 A,再根据 A 的结果查 B,再比较新旧版本或解释冲突”,才值得引入受控循环。
...
下一章会进入知识入库、数据治理与 RAG 生命周期。Agentic RAG 的在线循环只有在离线知识治理可靠时才有意义:文档要能解析、分级、继承权限、更新、删除、回源和评估。
