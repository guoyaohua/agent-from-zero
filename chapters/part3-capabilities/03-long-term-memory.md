...
![长期记忆写入评分与作用域过滤](../assets/part3-long-term-memory-write-score.svg)

这张图把长期记忆写入变成一个评分和过滤过程。候选信息先检查稳定性和复用价值,再检查来源可信度、隐私风险、scope 是否清楚、未来能否更新或删除。任何一项答不上来,都不应该高置信写入长期记忆。

长期记忆最好被视为“带来源的线索”,而不是未来任务里的最高优先级事实。当前用户明确要求、系统规则和权威工具观察都应优先于旧记忆。这样的优先级能避免长期记忆从便利功能变成隐藏偏见。
