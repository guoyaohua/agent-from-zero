...
记忆系统真正难调的地方,不是存储层,而是决策层。一次记忆相关行为至少包含四个判断:是否捕获、是否写入、是否召回、是否使用。每个判断都可能对未来行为产生影响,所以它们应该留下结构化账本。

![记忆决策账本](../assets/part3-memory-decision-ledger.svg)

账本不是为了多写日志,而是为了让系统能复盘三类问题:

- 为什么该记的信息没有被记住。
- 为什么不该用的旧记忆进入了上下文。
- 为什么同一条记忆在两个相似任务里结果不同。

一条有用的记忆决策记录通常包含:

| 字段 | 作用 | 例子 |
| --- | --- | --- |
| `candidate_id` | 串起捕获、写入、检索和使用 | `mem_cand_2026_07_07_001` |
| `decision` | 本阶段是接受、拒绝、降权还是待确认 | `write_rejected` |
| `reason_codes` | 机器可统计的原因 | `low_stability`, `missing_scope` |
| `source` | 信息来自哪里 | `explicit_user_request`, `tool_observation` |
| `scope` | 适用用户、项目、组织或任务 | `project:AIAgentHandbook` |
| `confidence` | 系统对这条记忆的可信度 | `0.82` |
| `ttl` | 何时过期或重新验证 | `30d` |
| `privacy_tags` | 是否涉及敏感信息 | `personal_preference` |
| `used_in_context` | 是否进入本次模型上下文 | `false` |

最容易被忽略的是“拒绝记录”。如果系统只记录写入成功的记忆,你只能看到最终留下来的东西,看不到被过滤掉的候选。上线后用户说“我明明告诉过它”,工程师却无法判断是没捕获、没写入、没召回,还是召回了但没使用。决策账本把这些失败拆开,记忆系统才有可调试性。

一个成熟做法是把账本同时用于离线评估和在线解释。离线看不同 `reason_codes` 的分布,可以发现写入规则是否过严或过松;在线给用户解释“这条偏好来自哪次明确请求,为什么今天没有使用”,可以降低记忆能力带来的不透明感。
...
记忆进入上下文后,模型会把它当作可见材料。问题是,模型不天然知道一条长期记忆应该低于当前用户请求,也不天然知道旧经验应该低于刚刚从工具拿到的观察。上下文工程必须显式标注优先级。

![记忆使用优先级阶梯](../assets/part3-memory-use-priority-ladder.svg)

一个稳妥的优先级大致是:

1. 系统规则、安全策略和开发者定义的不可违反边界。
2. 当前用户在本轮任务中的明确目标和约束。
3. 权威工具或数据库的当前观察。
4. 带来源、带时间、作用域匹配的 RAG 证据。
5. 长期记忆中的偏好、项目约定和历史经验。
6. 模型自己的常识和推断。

这条阶梯能解释很多看似奇怪的错误。例如用户长期偏好“回答简洁”,但本轮明确要求“逐步展开推导”,当前请求应覆盖旧偏好。又如长期记忆里写着“项目使用 npm”,但仓库文件和 CI 观察显示现在使用 pnpm,当前工具观察应覆盖旧记忆。

因此,记忆注入上下文时最好使用带边界的表达:

```text
长期记忆,低于当前用户请求和工具观察:
- 用户通常偏好简洁中文回答。source=explicit_user_request, updated_at=2026-06-01
- 旧项目约定:使用 npm。source=repository_observation, updated_at=2025-11-12, confidence=0.55
```

这比直接写“用户喜欢简洁回答,项目使用 npm”安全得多。前者告诉模型这是可参考线索,后者容易变成隐形指令。记忆系统越强,越要强调这一点:记忆不是上级命令,而是带来源、带置信度、可被新证据覆盖的历史线索。
